...
TestResourceRoute.post("/", zValidator("json", TestResourceSchema.schemaForCreateAndPatch), async (ctx) => {
...
TestResourceRoute.patch(
  ...
  async (ctx) => {
    ...
    const testResource = await db.query.TestResourceTable.findFirst({
      where: (clm, { eq, and, isNull }) => and(isNull(clm.deletedAt), eq(clm.id, id), eq(clm.projectId, project.id)),
    });
    if (!testResource) {
